...
#         base_name = os.path.basename(title).split('.')[0]
#         out_path = Path("outputs") / f"{base_name}_CAM.png"

#     cv2.imwrite(str(out_path), visualization_img)
#     print(f"[✅] CAM image saved to: {out_path}")

def show_cam_grad(grayscale_cam, src_img, title, out_path=None, img_path=None):
    """Fuse src_img and grayscale_cam, then save with image name suffix."""
    grayscale_cam = grayscale_cam[0, :]
    ...
    if out_path is None:
        ...
        img_name = Path(img_path).stem.replace(" ", "_").replace("(", "").replace(")", "")
        out_path = Path("outputs2") / f"cam_{title}_{img_name}.png"
...
def main():
    ...
    if args.config.endswith('.py'):
        ...
        model_cfg = getattr(user_config, 'model_cfg', None)
        train_pipeline = getattr(user_config, 'train_pipeline', None)
        val_pipeline = getattr(user_config, 'val_pipeline', None)
        data_cfg = getattr(user_config, 'data_cfg', None)
        lr_config = getattr(user_config, 'lr_config', None)
        optimizer_cfg = getattr(user_config, 'optimizer_cfg', None)
    else:
        model_cfg, train_pipeline, val_pipeline, data_cfg, lr_config, optimizer_cfg = file2dict(args.config)
    ...
    show_cam_grad(
        grayscale_cam, src_img, title=args.method, out_path=args.save_path,img_path=args.img)
